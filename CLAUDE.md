...
- Zeffy is unavailable as a contribution tool (it requires nonprofit status, which ROKT doesn't have).
- **Payment methods are additive, not either/or:** Zelle, Cash App, and mailed check are confirmed and currently live on the Sponsorship page (and the WordPress "How to Give" page). **Stripe** is the planned addition for card payments, pending Carol's confirmation — a Stripe placeholder was built then removed until she confirms. Stripe does not replace the other methods.
- **Sponsorship language rule (Carol, 2026-07-23; extended 2026-07-26).** ROKT is an LLC and can't offer tax-deductible giving, so **all user-visible contribution wording is sponsorship language**. Specifics:
  - Nav pill and Home card read **"Sponsorship"** — Carol's literal noun, chosen over the verb "Sponsor" at Geoff's direction. The card button reads "Sponsor with Credit Card" (verb, for grammar).
  - **Never reintroduce "Donate", "donation", "gift", or "give"** as user-visible copy. Carol: *"the language should be consistent throughout the website."* "Support Our Work" is still fine as a warm page-level heading.
  - **Two deliberate exceptions — do not "fix" these":** the Home *Virtual* card's "sessions **give** your child the same caring, focused instruction" (ordinary sense of the word, nothing to do with contributing), and the volunteer callout "Share your time, not just a sponsorship" (about time, not money).
  - **The route is `/sponsorship`** (renamed from `/donate` on 2026-07-23 at Geoff's direction). A 301 in `code/public/_redirects` keeps old links alive — **Next's `redirects()` config does nothing in a static export**, so host-level redirects are the only mechanism.
- **"Free" must always be qualified.** Children are vetted and selected by ROKT, so anywhere the site offers tutoring to a prospective parent it reads **"free to qualifying families"** (or "reach out to see if your child qualifies"), never an unconditional promise. Descriptive/sponsor-facing mentions ("support free K–5 reading tutoring") are fine — the tutoring genuinely is free; the qualification governs admission, not price.
- Still pending from Carol: Stripe confirmation, Venmo handle, real social media URLs, and whether the live WordPress site's "Donate" wording should be updated before cutover.
...
- **Phase B (active as of 2026-06-21, ahead of original schedule):** Migrate to Next.js static export. Originally planned to start ~10 months out (near the WordPress.com renewal), but Geoff confirmed an early start on 2026-06-21 — coding work begins now in Claude Code. See `todo/todo-nextjs-migration.md` and `todo/todo-copy-wordpress-content.md`. Static export is confirmed as the right call — ROKT has no SSR use cases — with a clean path to add user accounts later without a rewrite. At handoff, add a Git-based CMS (Decap or TinaCMS) so Carol can edit content herself without touching code.
  - **Progress (2026-07-23):** Cloudflare Pages is live and auto-deploys `main` → https://roots-of-knowledge.pages.dev/ (still the `.pages.dev` URL; `roots-of-knowledge.com` stays on WordPress until domain cutover). The site was rebuilt from a **Claude-Design redesign handoff** in `brain/redesign-handoff/`. **All 7 routes now exist** — Home, About, Programs, and Sponsorship are fully built; Enroll, Impact, and Volunteer ship as contact-first placeholders (`components/PlaceholderPage.tsx`). Fonts are Lora + Source Sans 3.
    - **Both of Carol's feedback rounds are implemented and deployed** — `9b05ded` (2026-07-23, `Webchanges.docx`: header sizes, copy edits, donation → sponsorship rename, `/donate` → `/sponsorship`) and `b38ca6b` (2026-07-26, her emailed answers: "free to qualifying families" on the home page, and gift/give → sponsorship throughout). See `MEMORY.md` for the full breakdown.
    ...
    - ⚠️ **Carol has asked to take over the site (2026-07-26)** — *"After you deploy the changes, I am now in the position to takeover."* **She is expecting WordPress-style editing, which this site does not have.** No admin login, no visual editor; it's a Next.js codebase in GitHub deploying through Cloudflare. The planned answer is a **Git-based CMS (Decap or TinaCMS)** — see the handoff section in `todo/todo-nextjs-design.md`. Scope it with her on a call before the domain cutover, not after.
